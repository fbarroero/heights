...
theorem infinitude_of_primes : ∀ N , ∃ p ≥ N , Nat.Prime p := by
  ...
  have hq : Nat.Prime q := by
    apply minFac_prime
    have hm : N ! > 0 := by exact factorial_pos N
    omega
  use q
  constructor
  ·
...
theorem not_prime_of_even (n : ℕ) (hne : n ≠ 2) (heven : n % 2 = 0) : ¬ Nat.Prime n := by
...
theorem infinitude_of_primes' : ∀ N, ∃ p ≥ N, Nat.Prime p := by
  ...
  { by_contra hqM
    ...
    have : (q ∣ 1) → False := by exact fun a ↦ (fun {a} h ↦ (iff_false_intro h).mp) hqn1 hq1
    apply this
    exact hq1
     }
