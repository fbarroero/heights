...
theorem card_eq_of_natDegree_le_of_coeff_le {n : ℕ} (hn : 1 ≤ n) {B₁ B₂ : Fin n → ℝ}
    ...
    ∏ i : Fin n, (Int.floor (B₂ i) - Int.ceil (B₁ i) + 1)  := by
...
theorem card1 {n : ℕ} (hn : 1 ≤ n) (B : NNReal) :
  ...
  have h_B (i : Fin n) : ⌈B₁ i⌉ ≤ ⌊B₂ i⌋ := by
    simp only [Int.ceil_neg, neg_le_self_iff, Int.le_floor, Int.cast_zero, B₁, B₂]
  ...
  have (i : Fin n) : (⌊(n.choose i) * B ^ (n - i)⌋₊ : ℤ) = ⌊(n.choose i) * (B : ℝ) ^ (n - i)⌋ := by
    ...
    exact this i
  conv => enter [2,2]; ext i; enter [1]; rw [this, Int.two_mul, ← sub_neg_eq_add, ← Int.ceil_neg]
...
def funct (n : ℕ) (B : NNReal) :
  ...
  rw [id_eq]
  refine ⟨h_deg, ?_⟩
  by_cases h_p : p = 0; simp only [h_p, coeff_zero, abs_zero, Int.cast_zero]; norm_cast; simp
  intro i
  by_cases h_i : p.natDegree < i; simp only [coeff_eq_zero_of_natDegree_lt h_i, abs_zero,
    Int.cast_zero]; norm_cast; simp
  norm_cast
  have h_norm : |p.coeff i| = (‖p.coeff i‖₊ : ℝ) := by
    simp only [Int.cast_abs, coe_nnnorm]
    rfl
  rw [h_norm]
  have h_deg_eq : (map (Int.castRingHom ℂ) p).natDegree =  p.natDegree := by
    simp only [natDegree_map_eq_iff, eq_intCast, ne_eq, Int.cast_eq_zero, leadingCoeff_eq_zero]
    exact Decidable.not_or_of_imp (congrArg natDegree)
  have h_nnnorm (j : ℕ) : ‖p.coeff j‖₊ = ‖(map (Int.castRingHom ℂ) p).coeff j‖₊ := by
    rw [← Complex.nnnorm_intCast]
    congr
    simp
  rw [h_nnnorm]
  have h_split : Splits (RingHom.id ℂ) (map (Int.castRingHom ℂ) p) :=
    IsAlgClosed.splits (map (Int.castRingHom ℂ) p)
  have h_one_le : 1 ≤ ‖(map (Int.castRingHom ℂ) p).leadingCoeff‖₊ := by
    rw [leadingCoeff, ← h_nnnorm, h_deg_eq, ← NNReal.natCast_natAbs, ← leadingCoeff]
    norm_cast
    exact Nat.one_le_iff_ne_zero.mpr (Int.natAbs_ne_zero.mpr (leadingCoeff_ne_zero.mpr h_p))
  norm_cast
  refine le_trans (bdd_coeff_of_bdd_roots_and_leading_coeff h_split i) ?_
  rw [mul_comm ‖(map (Int.castRingHom ℂ) p).leadingCoeff‖₊, mul_assoc]
  ...
  · exact le_trans (leading_coeff_le_mahler_measure (map (Int.castRingHom ℂ) p)) bound
  · apply le_trans _ bound
    exact le_trans h_one_le (leading_coeff_le_mahler_measure (map (Int.castRingHom ℂ) p))
